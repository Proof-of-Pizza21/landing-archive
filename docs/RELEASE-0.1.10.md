# Activity, collection, and backups — 0.1.10
...
schema: backup triggers, views, generated columns, and code are not installed.
Native SQLite memory is limited to 64 MiB and the verification process has a
separate 256 MiB limit for JavaScript memory. Files are verified before database
changes; a transaction error leaves previous history intact. An interruption
