...
assert.match(version || '', /^0\.1\.\d+$/, 'Only 0.1.x previews are enabled');
assert.equal(pkg.version, version, 'Requested version differs from package.json');
const dockerVersion = readFileSync('Dockerfile', 'utf8').match(/org\.opencontainers\.image\.version="([^"]+)"/)?.[1];
const workerVersion = readFileSync('src/version.ts', 'utf8').match(/appVersion\s*=\s*['"]([^'"]+)/)?.[1];
assert.equal(dockerVersion, version, 'Container label differs from package.json');
assert.equal(workerVersion, version, 'Worker version differs from package.json');
