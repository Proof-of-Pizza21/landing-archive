...
test('opening a previous archive preserves pending jobs while adding manual scheduling', async () => {
  ...
  try {
    ...
    assert.equal(job.manual, 0); assert.ok(db.prepare('PRAGMA table_info(versions)').all().some(column => column.name === 'quality')); assert.ok(db.prepare('PRAGMA table_info(pages)').all().some(column => column.name === 'ignore_rules'));  assert.equal(get('PRAGMA user_version')!.user_version, 3);
