...
test('offline browser navigation preserves dates, blocks live traffic and keeps the archive private', async () => {
  ...
  const save = async (url: string, title: string, day: number, html: string) => {
    ...
    const result = await recordCapture(page, row, { requestedUrl: url, finalUrl: url, title, text: title, statusCode: 200, html, screenshot: PNG.sync.write(png), headings: [], links: [], imageUrls: [], warnings: [], quality: { version: 2, stable: true, renderStatus: 'complete', archiveStatus: 'complete', status: 'complete', missingImages: 0, reasons: [] },  capturedAt: `2026-09-0${day}T12:00:00.000Z` });
